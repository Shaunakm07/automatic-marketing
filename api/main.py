...
async def regenerate_draft(item_id: str, body: RegenerateRequest, background_tasks: BackgroundTasks):
    """
    Mark the existing draft as rejected and queue a background job that rewrites it.
    The new draft appears in Supabase once the background task completes.
    """
    from integrations import supabase_store
    ...
    def _regen(draft: dict, instructions: str):
        from agents import blog_agent, research_agent
        import json
        ...
        digest = {
            ...
            "key_findings":    _json.loads(digest_row["key_findings"])
    ...
    background_tasks.add_task(_regen, draft, body.instructions)
